...
import {AMMProtocolShouldNotBeAllRemoved} from "../../src/interfaces/IGuard.sol";

contract BittyGuardTest is Test {
    BittyGuard public bittyGuard;
    ...
    function setUp() public {
        ...
        // `BittyGuard` grants `DEFAULT_ADMIN_ROLE` to `tx.origin`; align origin with a fixed admin for stable tests.
        address deployAdmin = makeAddr("deployAdmin");
        vm.startPrank(deployAdmin, deployAdmin);
        bittyGuard = new BittyGuard();
        vm.stopPrank();
    ...
    function test_InitializePopulatesGetters() public {
        address deployAdmin = makeAddr("initAdmin");
        vm.startPrank(deployAdmin, deployAdmin);
    ...
    function test_GetBittyGuardInitCode() public pure {
        bytes memory bytecode = type(BittyGuard).creationCode;
